...
export const startDateSelector = createSelector(calendarFeature, cal => {
  return cal.startDate;
});

export const isDraggingSelector = createSelector(
  calendarFeature,
  cal => !!cal.inFlight || !!cal.original
);
...
export const updatedSelector = createSelector(originalSelector, distanceSelector, (orig, diff) => {
  return orig ? orig.applyDiff(diff) : null;
});

const notSetWeek = new Week();

export const weeksSelector = createSelector(startDateSelector, startDate => {
  let four = List([
  ...
  while (lastDayCovered.isBefore(lastDayOfMonth, 'day')) {
    const newWeek = Week.from(startDate.clone().add({ weeks: i++ }));
...
export const visibleEvents = createSelector(eventSelector, weeksSelector, (events, weeks) => {
...
export const allEventSelector = createSelector(
  ...
  (events, inFlight, updated) => {
    ...
    events = updated
      ? events.splice(
          events.findIndex(e => e.uniqueId === updated.uniqueId),
          1,
          updated
        )
      : events;
