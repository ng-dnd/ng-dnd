...
@Component({
  ...
  imports: [DndModule, CalendarEvent, AsyncPipe],
})
export class CalendarDay implements OnInit, OnDestroy, AfterViewInit {
  ...
  events$!: Observable<List<CalendarEventModel>>;
  ...
  intradayEvent() {
    this.store.dispatch(
      new NewEvent(CalendarEventModel.standard(`Meeting with ${faker.name.findName()}`, this.day))
    );
  }
  ...
  unique(_: number, e: CalendarEventModel) {
    return e.uniqueId;
  }
}
