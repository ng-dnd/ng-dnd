...
@Component({
  ...
  template: `
    ...
    <p>This example does the same list operations as the 'Simple list' example, but it does them with
      <code>immer</code> just to shake things up.</p>
    <p>It also uses the <code>[ngDndSortableExternal]</code> directive to allow dragging in items that
