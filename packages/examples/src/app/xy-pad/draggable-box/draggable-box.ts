...
import { Spot, SpotModel } from '../spot';
...
@Component({
  ...
  imports: [DndModule, Spot, Crosshairs, AsyncPipe],
})
export class DraggableBox implements OnInit, OnDestroy {
  private dnd = inject(DndService);

  @Input() spot!: SpotModel;
  @Output() endDrag = new EventEmitter<SpotModel>();

  source = this.dnd.dragSource<SpotModel>('SPOT', {
    beginDrag: () => {
      return this.spot;
    },
    isDragging: () => {
